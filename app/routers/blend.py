...
@router.post("/api/v1/blend-by-url-test")
async def blend_images_by_url_test(
    request: BlendByUrlRequest = Body(..., description="图片混合URL请求参数"),
    api_token: str = Depends(get_current_api_token)
):
    """
    混合两张URL图片（测试端点）并上传到AIGC网盘
    """
    try:
        # 处理相对路径，转换为完整URL
        base_image_url = request.base_image_url
        ...
        if base_image_url.startswith('/examples/'):
            base_image_url = f"http://localhost:58889{base_image_url}"
        if blend_image_url.startswith('/examples/'):
            blend_image_url = f"http://localhost:58889{blend_image_url}"

        base_contents, base_content_type = ImageUtils.download_image_from_url(base_image_url)
        blend_contents, _ = ImageUtils.download_image_from_url(blend_image_url)
        result_bytes = BlendService.blend_images(
            base_image_bytes=base_contents,
            blend_image_bytes=blend_contents,
            blend_mode=request.blend_mode,
        ...
        parameters = {
            ...
            "blend_image_url": blend_image_url
        }
