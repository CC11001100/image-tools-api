...
@router.post("/api/v1/resize-by-url-test")
async def resize_image_by_url_test(
    image_url: str = Form(...),
    width: Optional[int] = Form(None),
    height: Optional[int] = Form(None),
    maintain_aspect: Optional[bool] = Form(True),
    ...
    try:
        ...
        if image_url.startswith('/'):
            # 相对路径，转换为本地文件路径
            import os
            file_path = os.path.join(os.getcwd(), "public" + image_url)
            if os.path.exists(file_path):
                with open(file_path, 'rb') as f:
                    contents = f.read()
            else:
            ...
            # 完整URL，下载图片
            contents, _ = ImageUtils.download_image_from_url(image_url)
        ...
        return ApiResponse.success(
            message="图片缩放处理完成",
            data={
                "file_info": dummy_file_info.dict(),
                "processing_info": {
                    **parameters,
